...
| CA-07 | Si pnpm no se puede activar automáticamente, el script muestra error y se detiene | Simular fallo de corepack: bloquear la red o deshabilitar corepack temporalmente (`corepack disable pnpm` y luego `$env:COREPACK_ENABLE_STRICT="0"`). Ejecutar `run.ps1`. Debe mostrar "No se pudo activar pnpm" y hacer pausa. Restaurar con `corepack enable pnpm`. | 🟡 Alta |
