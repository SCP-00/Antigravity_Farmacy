...
- Set root `.env` as the source of truth for environment values, with `backend/.env` as fallback (raíz sobrescribe).
- `env.ts` carga ambos: primero `backend/.env` luego raíz `.env` (raíz tiene prioridad final).
...
- Existing tooling expecting `backend/.env` keeps working due to dual-loading.
- Root `.env` siempre sobreescribe `backend/.env` si ambos existen, garantizando que `AGENTS.md` siga siendo correcta ("keep edits in root .env").
