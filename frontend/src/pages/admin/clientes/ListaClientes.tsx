    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="surface overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-red-500">Error al cargar clientes</div>
        ) : clientes.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {clientes.map((c: any) => (
                  <tr key={c.id} onClick={() => navigate(`/admin/clientes/${c.id}`)} className="hover:bg-slate-50/70 cursor-pointer transition-colors">
                    ...
                    <td className="px-5 py-4 text-gray-600">
                      ...
                      {c.telefono && <div className="flex items-center gap-1.5 mt-1 text-xs text-gray-500">{c.telefono}</div>}
                    </td>
                    <td className="px-5 py-4 text-center">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-amber-50 text-amber-700 font-bold text-xs border border-amber-200">
                    ...
                    <td className="px-5 py-4 text-right text-gray-500 text-xs flex justify-end items-center gap-1">
        ...
        {meta.totalPaginas > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 bg-slate-50">
            <p className="text-xs text-gray-500">Página {pagina} de {meta.totalPaginas}</p>
